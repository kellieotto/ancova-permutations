---
title: "ANCOVA Comparison Simulations: Skewed, Poisson Data"
...
$\beta_0$ is the coefficient for the baseline measurement $Y_{i0}$, $\beta_j$ is the mean effect of being at stratum $j$, $Z_{ij}$ is the treatment level indicator,, $\gamma_j$ is the effect of treatment at stratum $j$, and $\varepsilon_{ij}$ takes the values $0$ or $0.5$ with equal probability.
...
* Design 2: $\gamma_1 = \gamma > 0$, $\gamma_2 = \gamma_3 = 0$. This is a constant, additive treatment effect at stratum 1, but no treatment effect at strata 2 and 3. This is a simplistic case of heterogeneous treatment effects.
...
```{r generate_sample}
generate_outcome <- function(baseline, tr_effect, stratum_effect, error){
  Y1 <- baseline + tr_effect + stratum_effect + error
  Y1 <- floor(Y1)
...
generate_simulated_data <- function(gamma, effect, n = c(16, 16, 16)){
  ...
  Y1 <- generate_outcome(Y0, gamma_vec*Z, stratum_effect, epsilon)
...
design0_pvalues <- replicate(1000, {
  ...
  tmp$Y1 <- generate_outcome(tmp$Y0, rep(0, nrow(tmp)), tmp$stratum_effect, tmp$epsilon)
...
plot_power_curves(design0_power, "No Effect, Balanced Design")

plot_pvalue_hist(design0_pvalues, "No Effect, Balanced Design")
```
...
design1_pvalues <- replicate(1000, {
  ...
  tmp$Y1 <- generate_outcome(tmp$Y0, tmp$Z*tmp$gamma_vec, tmp$stratum_effect, tmp$epsilon)
  generate_simulated_pvalues(tmp)
})
...
## Design 2: Single stratum effect
...
tmp <- generate_simulated_data(gamma = 0.5, effect = "single stratum effect")
...
design3_pvalues <- replicate(100, {
  tmp$Z <- permute_within_groups(tmp$Z, tmp$stratumID)
  tmp$epsilon <- rbinom(nrow(tmp), 1, 0.5) - 0.5
  tmp$Y1 <- generate_outcome(0.1*tmp$Y0, tmp$Z*tmp$gamma_vec, tmp$stratum_effect, tmp$epsilon)
  generate_simulated_pvalues(tmp)
})
...
```{r design_null_imbalanced, fig.height = 4}
set.seed(760682460) # Generated from random.org Timestamp: 2016-11-14 10:21:12 UTC

tmp <- generate_simulated_data(gamma = 0, effect = "same effect", n = c(8, 16, 24))
...
```{r design_null_imbalanced_plots, fig.height = 4, fig.align="center"}
plot_power_curves(design00_power, "No Effect, Imbalanced design")

plot_pvalue_hist(design00_pvalues, "No Effect, Imbalanced design")
```
...
# Design 5: Single stratum effect, imbalanced design
...
tmp <- generate_simulated_data(gamma = 0.5, effect = "single stratum effect", n = c(8, 16, 24))
...
p5 <- plot_power_curves(design5_power, "Effect of 0.5 at a Single stratum, Imbalanced Design")
p5 
plot_pvalue_hist(design5_pvalues, "Effect of 0.5 at a Single stratum, Imbalanced Design")
```
...
powers <- list(design1_power %>% as.data.frame() %>% mutate("Treatment" = rep("Constant Additive Effect", nrow(design1_power)), "Design" = rep("Balanced", nrow(design1_power))), 
               design2_power %>% as.data.frame() %>% mutate("Treatment" = rep("Single Stratum Effect", nrow(design2_power)), "Design" = rep("Balanced", nrow(design2_power))), 
               ...
               design5_power %>% as.data.frame() %>% mutate("Treatment" = rep("Single Stratum Effect", nrow(design5_power)), "Design" = rep("Imbalanced", nrow(design5_power))),
               design0_power %>% as.data.frame() %>% mutate("Treatment" = rep("No Effect", nrow(design0_power)), "Design" = rep("Balanced", nrow(design0_power))),
               design00_power %>% as.data.frame() %>% mutate("Treatment" = rep("No Effect", nrow(design00_power)), "Design" = rep("Imbalanced", nrow(design00_power))))

all_power_curves <- do.call(rbind, powers)
twobytwo <- all_power_curves %>%
  filter(Treatment != "No Effect") %>%
  melt(id.vars = c("Treatment", "Design"))  %>% 
...
pdf(file = "../ms/fig/skewed_simulation_power.pdf", width = 8)
twobytwo
dev.off()
```
...
summarytab <- xtable(summary05, digits = 3, align = "rp{1.25in}|p{0.7in}|p{0.6in}p{0.8in}p{0.8in}p{0.8in}p{0.75in}",
       caption = "Empirical power at level $0.05$ for discrete, skewed simulated data",
       label = "tab:skewed_power")
print(summarytab, hline.after = c(-1, 0, 3, nrow(summarytab)), type="latex", file="../ms/fig/skewed_simulation_power_summary.tex") 
summarytab 
```