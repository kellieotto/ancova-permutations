...
In clinical trials, the standard method of measuring the effectiveness of one treatment over another is parametric analysis of covariance (ANCOVA), a special case of linear regression which incorporates stratification.
The hypothesis test is derived based on strong, untestable assumptions about the data-generating process such as a constant treatment effect across groups and Gaussian, homoskedastic random errors.
...
\parttitle{Methods}
Permutation tests are a nonparametric hypothesis test and require only minimal assumptions which are often guaranteed by the randomization that was conducted.
In contrast to parametric tests, which rely up on strong assumptions to obtain a null distribution with a simple closed form, one uses the assumption of exchangeability or randomization to simulate approximate null distributions for permutation tests.
These tests are exact, even in finite samples.
We compare ANCOVA to several permutation tests based on linear models, which control for pretreatment covariates to increase power and precision.

\parttitle{Results}
We simulate randomized trials using a variety of data-generating processes, some of which violate the parametric ANCOVA assumptions.
...
Techniques have been developed to deal with each of these issues, but solutions are rarely as simple as an asymptotic normality argument.
In this paper, we focus on the first question of heterogeneity and assume that the others do not affect the study.
