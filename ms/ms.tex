...
Statistical test of treatment effects based on the linear model, such as analysis of variance (ANOVA) and analysis of covariance (ANCOVA), are widely accepted by organizations that regulate clinical trials.
In this paper, we focus on ANCOVA, a special case of linear regression which incorporates stratification.
Traditional ANCOVA is derived using strong assumptions about the data-generating process so that the resulting inference is based on parametric distributions.
Because these methods are well understood and robust, they are sometimes applied to data that depart from the method's assumptions, such as ordinal integer scores.
Given the considerable costs and scientific importance of clinical trials, it is desirable to include an additional methodology which is potentially more robust to departures from assumptions as a check on the statistical inference for the main study endpoints.
%\parttitle{Methods}
Permutation tests are a nonparametric alternative that require only minimal assumptions which are often guaranteed by the randomization that was conducted.
In contrast to parametric tests, which rely up on strong assumptions to obtain a null distribution with a simple closed form, permutation tests use the assumption of exchangeability or randomization to obtain null distributions directly by considering what would have been observed under different possible randomizations.
We compare parametric ANCOVA to several permutation tests based on linear models, which control for pretreatment covariates to increase power and precision.
%\parttitle{Results}
We simulate randomized trials using a variety of data-generating processes, some of which violate the parametric assumptions.
The permutation tests based on linear models maintain power comparable to the ANCOVA in every case.
We illustrate the use of these permutation tests alongside the parametric ANCOVA with data from a clinical trial conducted across eight sites,
each having a different numbers of patients.
%\parttitle{Conclusions}
These results suggest that a linear model permutation test may serve as a secondary analysis to give strength to the statistical inferences from the primary analyses of a clinical data study.
